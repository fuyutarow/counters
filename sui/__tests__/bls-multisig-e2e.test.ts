/**
 * BLS Multisig End-to-End Integration Test
 *
 * This test demonstrates the complete BLS multisig flow:
...
import { type Bls_multisig_counterBlsMultisigCounterType, counterPackage } from "@/abi";
...
const getCounterValue = async (client: SuiClient, counterId: string): Promise<number> => {
  ...
  const fields = counterObject.data.content.fields as Bls_multisig_counterBlsMultisigCounterType;
...
const getRealSealShardPublicKeys = async (
  ...
  for (const keyServerId of keyServerIds) {
    try {
      ...
      // Ensure scalar is within valid range by taking modulo of the curve order
      ...
      // BLS12-381 scalar field order (reduced to ensure valid scalar)
      const fieldOrder = BigInt(
        "0x73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001",
      );
      const validScalar = scalarBigInt % fieldOrder;

      const g2Point = g2Generator.multiply(validScalar);
      const publicKey = g2Point.toBytes(); // Use toBytes() method

      publicKeys.push(publicKey);
    } catch (error) {
      throw new Error(`Failed to generate public key for ${keyServerId}: ${error}`);
    }
  }
...
/**
 * Create BLS multisig counter with real Seal Key Server public keys
 */
const createBlsMultisigCounter = async (
  client: SuiClient,
  ...
  counterPackage.bls_multisig_counter.share(tx, {
    arguments: [
  ...
  const created = result.objectChanges?.find(
    (change) => change.type === "created" && change.objectType?.includes("BlsMultisigCounter"),
  );

  if (!created || created.type !== "created") {
    throw new Error("Failed to find created BLS multisig counter");
  }
...
const fetchSecretKeyShares = async (
  ...
  try {
    ...
    // Debug: Log values before SessionKey.create
    console.log("Debug SessionKey.create parameters:");
    console.log("- address:", signerAddress);
    console.log("- packageId:", COUNTER_PACKAGE_ID);
    console.log("- packageId length:", COUNTER_PACKAGE_ID.length);
    console.log("- ttlMin: 10");
    console.log("- signer type:", typeof signerKeypair);
    ...
    approveTx.moveCall({
      target: `${COUNTER_PACKAGE_ID}::bls_multisig_counter::seal_approve`,
      arguments: [
    ...
    console.log("Debug getDerivedKeys parameters:");
    console.log("- id:", ibeIdHex);
    console.log("- id length:", ibeIdHex.length);
    console.log("- threshold:", requiredCount);
 ...
 */
const createBlsMultisigSignature = (
...
const generateBLSSignature = async (
  ...
  try {
    ...
    const signature = createBlsMultisigSignature(
...
const verifySignatureAndCreateProof = async (
  ...
  const [aggregatedKey] = counterPackage.bls_multisig_counter.new_bls_aggregated_pk(tx, {
  ...
  for (const keyServerId of keyServerIds) {
    counterPackage.bls_multisig_counter.aggregate_seal_shard_pubkey(tx, {
      arguments: [tx.object(counterId), aggregatedKey, tx.pure.id(keyServerId)],
    });
  }

  // Step 3: Verify signature and create proof
  const [proof] = counterPackage.bls_multisig_counter.verify_and_create_proof(tx, {
    arguments: [
      tx.object(counterId),
      aggregatedKey,
  ...
  counterPackage.bls_multisig_counter.increment(tx, {
    arguments: [tx.object(counterId), proof],
  });

  // Step 5: Clean up AggregatedPublicKey
  counterPackage.bls_multisig_counter.destroy_bls_aggregated_pk(tx, {
...
describe("BLS Multisig End-to-End Integration", () => {
  ...
  test("Step 1: creates BLS multisig counter with real Seal Key Server public keys", async () => {
    counterId = await createBlsMultisigCounter(client, keypair);
  ...
  test("Integration: completes full BLS multisig flow (create → sign → verify → increment)", async () => {
    // Create new counter for clean integration test
    const integrationCounterId = await createBlsMultisigCounter(client, keypair);
